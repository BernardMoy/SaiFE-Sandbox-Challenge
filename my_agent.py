...
class Agent:
    def __init__(self, config):
        ...
        # Wider than tau/2 so we spend less time sitting at an edge.
        self.width = max(1, int(self.tau * 0.8))
        # Rebalance only once price actually reaches (or passes) a band edge.
        self.edge_buffer = 0
        # How far to shift the band toward the side the pool price is being
        # pushed, based on the gap between pool price and external midprice.
        self.skew = max(1, self.tau // 5)

    def get_action(self, state):
        n = state["sqrt_price"].shape[0]

        # Pool price vs. external "fair value": positive gap means the pool
        # is underpriced, so arbitrage buying will push its price (and tick)
        # up; negative gap means overpriced, so selling will push it down.
        pool_price = state["sqrt_price"] ** 2
        direction = np.sign(state["midprice"] - pool_price)

        # Shift the band toward the side the price is heading, leaving more
        # room on that side before falling out of range.
        shift = direction * self.skew
        lower = np.clip(-self.width + shift, -self.tau, self.tau - 1)
        upper = np.clip(self.width + shift, -self.tau + 1, self.tau)
        upper = np.where(upper <= lower, lower + 1, upper)

        # Rebalance once price has reached (or passed) either edge of the
        # current position, or on the very first step.
        ...
        out_of_range = (room_below <= self.edge_buffer) | (room_above <= self.edge_buffer)

        ever_deployed = state.get("lp_ever_deployed", np.zeros(n, dtype=bool))
        rebalance = out_of_range | ~ever_deployed
